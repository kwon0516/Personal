# -*- coding: utf-8 -*-
"""
AutoHotCorner v5 (PyQt5 single-file)
Changes from v4 based on user spec:
1) Hotcorner detection always-on in background (app running ⇒ detection active).
2) Start/Stop controls ONLY the periodic keepalive key (monitor sleep prevention).
   - Status pill shows Keepalive On/Off.
   - Tray toggle also controls keepalive.
All previous fixes remain (CapsLock/NumLock, modal dialogs, styling, logs).
"""
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        # Keepalive state (separate): hotcorner is always active; keepalive may be on/off.
        ...
        ab_l.addWidget(QLabel(f"{APP_NAME} v{VERSION} — 핫코너 / 절전방지"))
