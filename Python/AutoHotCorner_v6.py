# -*- coding: utf-8 -*-
"""
AutoHotCorner v6 (PyQt5 single-file)
...
- NEW:
  1) App icon (AutoHotKeyIcon.ico) for window & tray (use resource_path)
  2) "Windows 시작 시 자동 실행" option (registry Run key, per-user)
  3) "프로그램 시작 시 Keepalive 자동 시작" option
  4) "프로그램 시작 시 트레이로 자동 최소화" option
"""
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        ab_l.addWidget(QLabel(f"{APP_NAME} v{VERSION} — For Windows Hotcorner + Keepalive Monitor"))
    ...
    def toggle_keepalive(self):
        ...
        if turn_on:
            ...
            if key != "none":
                ...
                self._do_keepalive()  # immediate send
            else:
