# -*- coding: utf-8 -*-
"""
AutoHotCorner v2 (PyQt5, single-file)
- Fully coded UI (no .ui)
- Modern dark theme, rounded "card" sections, clearer status pill
- Tabbed layout: Dashboard / Hotcorners / Calibration / Settings
...
from PyQt5.QtCore import Qt, QSize, QTimer, QPoint, QEvent
...
from PyQt5.QtWidgets import (
    ...
    QSystemTrayIcon, QMenu, QLineEdit, QFileDialog, QTabWidget, QStyle, QFrame
)
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        # Header bar
        top = QWidget()
        th = QHBoxLayout(top); th.setContentsMargins(12,8,12,8)
        self.lbl_title = QLabel(f"<b>{APP_NAME}</b>")
        self.lbl_title.setFont(QFont(self.font().family(), 14, QFont.Bold))
        self.lbl_status = QLabel(status_pill("Stop", False))
        th.addWidget(self.lbl_title); th.addStretch(1); th.addWidget(self.lbl_status)
        wrapper = QVBoxLayout(); wrapper.setContentsMargins(10,10,10,10)
        container = QWidget()
        container.setLayout(wrapper)

        # Build tabs
        self.page_dash = self._build_dashboard()
        self.page_hot = self._build_hotkeys()
        self.page_cal = self._build_calibration()
        self.page_set = self._build_settings()
        ...
        # outer layout
        root = QWidget()
        outer = QVBoxLayout(root); outer.setContentsMargins(8,8,8,8)
        outer.addWidget(top)
    ...
    def _build_dashboard(self):
        ...
        for k in ["f15","scrolllock","shift","ctrl","alt","win","tab","enter","esc","space","none"]:
        ...
        lay.addLayout(form); lay.addStretch(1); lay.addWidget(self.btn_toggle)
        v.addWidget(card)
    ...
    def _build_calibration(self):
        ...
        gl.addWidget(self.chk_mon1, 0, 0)
        gl.addWidget(self.chk_mon2, 0, 1)

        gl.addWidget(QLabel("허용 오차(px):"), 1, 0)
        self.tol_edit = QLineEdit(str(self.cfg.get("tolerance_px",0))); self.tol_edit.setFixedWidth(80)
        gl.addWidget(self.tol_edit, 1, 1)
    ...
    def _build_settings(self):
        ...
        gl.addWidget(QLabel(f"설정 경로: {config_path()}"), 0, 0, 1, 3)
        gl.addWidget(self.btn_save, 1, 0)
        gl.addWidget(self.btn_export, 1, 1)
        gl.addWidget(self.btn_import, 1, 2)
        gl.addWidget(self.btn_reset, 1, 3)
    ...
    def _start_calibration(self):
        ...
        info.setWindowTitle("좌표 보정")
        info.setText("원하는 위치에 마우스를 두고 Enter, 취소는 ESC")
        info.setStandardButtons(QMessageBox.Ok | QMessageBox.Cancel)
        info.button(QMessageBox.Ok).setText("대기")
        info.button(QMessageBox.Cancel).setText("취소")
        info.show()

        self._calib_waiting = True

        def event_filter(obj, ev):
            if ev.type() == QEvent.KeyPress and self._calib_waiting:
                if ev.key() in (Qt.Key_Return, Qt.Key_Enter):
                    ...
                    info.accept()
                    self.removeEventFilter(self._ef)
                    return True
        ...
        self._ef = type("EF",(object,),{"eventFilter":event_filter})()
        self.installEventFilter(self._ef)
    ...
    def _reset_cfg(self):
        self.cfg = load_config().__class__()  # not correct; fallback to default below
        self.cfg = {
            "selected_key": "f15",
            "cycle_min": 1,
            "use_monitor": [False, False],
        ...
        save_config(self.cfg)
        # refresh UI
        self.combo_key.setCurrentText("f15")
