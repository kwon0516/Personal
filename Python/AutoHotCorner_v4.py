# -*- coding: utf-8 -*-
"""
AutoHotCorner v4 (PyQt5 single-file)
Fixes & Improvements:
1) Removed background for "주기 키" input widgets.
2) QMessageBox text forced to black for readability.
3) Key-capture dialog requires Enter/확인 to commit; supports combos (Ctrl/Alt/Shift/Win + Key).
4) Added "최근 실행" log label and improved corner trigger reliability.
5) Keepalive: added "즉시 실행" 버튼 and "테스트(초)" field for quick verification.
"""
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        ctrl = QGroupBox("컨트롤"); lay = QGridLayout(ctrl)
        ...
        self.spin_test_sec = QSpinBox(); self.spin_test_sec.setRange(1, 60); self.spin_test_sec.setValue(5)
        ...
        lay.addWidget(QLabel("테스트(초):"),2,0); lay.addWidget(self.spin_test_sec,2,1)
        lay.addWidget(self.btn_keepalive_now,2,2)
