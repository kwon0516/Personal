# -*- coding: utf-8 -*-
"""
AutoHotCorner v10 (Ripple + Visual Customizable + Full UI Settings)
Baseline: v9 final + ripple indicator (mac-style) + ripple customization (color/opacity/shape/thickness)
"""
...
def default_config() -> dict:
    return {
        ...
        # Ripple effect settings (New in v10)
        "ripple_enabled": True,
        "ripple_size_px": 120,
        "ripple_duration_ms": 450,
        "ripple_color": "#5A82FF",
        "ripple_stroke_px": 4,
        "ripple_stroke_opacity": 60,   # %
        "ripple_fill_opacity": 15,     # %
        "ripple_shape": "circle"       # circle or square
    }
...
class RippleOverlay(QWidget):
    """A frameless, translucent overlay that shows an expanding/fading ring at (x,y)."""
    def __init__(self, parent=None):
        ...
        # Fullscreen overlay across the virtual desktop
        # On multi-monitor, you may want per-screen windows; here we simply use the primary virtual geometry.
        rec = QApplication.desktop().geometry()
        self.setGeometry(rec)
        self.hide()

    def configure(self, max_radius:int, duration_ms:int):
        self._max_radius = max(20, int(max_radius))
        self._duration_ms = max(100, int(duration_ms))

    def show_ripple(self, x:int, y:int):
    ...
    def _tick(self):
        ...
        # Ease-out curve for smoothness
        curve = QEasingCurve(QEasingCurve.OutCubic)
        eased = curve.valueForProgress(t)
    ...
    def paintEvent(self, ev):
        ...
        # Draw soft outer ring
        color = QColor(90, 130, 255, int(255 * self._opacity * 0.6))
        pen = QPen(color, 4)
        p.setPen(pen)
        p.setBrush(Qt.NoBrush)
        p.drawEllipse(self._center, self._radius, self._radius)

        # Fill inner faint circle
        fill = QColor(90, 130, 255, int(255 * self._opacity * 0.15))
        p.setPen(Qt.NoPen)
        p.setBrush(QBrush(fill))
        inner = max(0, self._radius - 12)
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        try:
            w, h = self.cfg.get("window_size", [620, 400])
        ...
        # ripple options
        self.chk_ripple = QCheckBox("핫코너 시각 효과(리플) 사용")
        self.chk_ripple.setChecked(bool(self.cfg.get("ripple_enabled", True)))
        self.spin_ripple_size = QSpinBox(); self.spin_ripple_size.setRange(30, 1000); self.spin_ripple_size.setValue(int(self.cfg.get("ripple_size_px",120)))
        self.spin_ripple_dur  = QSpinBox(); self.spin_ripple_dur.setRange(100, 5000); self.spin_ripple_dur.setValue(int(self.cfg.get("ripple_duration_ms",450)))
        self.spin_ripple_size.setSuffix(" px")
        self.spin_ripple_dur.setSuffix(" ms")
        ...
        gls.addWidget(self.chk_ripple, 7, 0, 1, 2)
        gls.addWidget(QLabel("리플 크기:"), 8, 0)
        gls.addWidget(self.spin_ripple_size, 8, 1)
        gls.addWidget(QLabel("리플 지속시간:"), 8, 2)
        gls.addWidget(self.spin_ripple_dur, 8, 3)
        ...
        self.overlay.configure(int(self.cfg.get("ripple_size_px",120)), int(self.cfg.get("ripple_duration_ms",450)))
    ...
    def _maybe_ripple(self, x:int, y:int):
        if not self.chk_ripple.isChecked():
            return
        self.overlay.configure(int(self.spin_ripple_size.value()), int(self.spin_ripple_dur.value()))
        self.overlay.show_ripple(int(x), int(y))

    def _trigger_corner(self, idx: int, visual_x=None, visual_y=None):
        # show ripple first (if enabled)
        if visual_x is not None and visual_y is not None:
            self._maybe_ripple(visual_x, visual_y)
    ...
    def _save_clicked(self):
        ...
        # ripple
        self.cfg["ripple_enabled"] = self.chk_ripple.isChecked()
        self.cfg["ripple_size_px"] = int(self.spin_ripple_size.value())
        self.cfg["ripple_duration_ms"] = int(self.spin_ripple_dur.value())
    ...
    def _import_cfg(self):
        ...
        try:
            ...
            self.chk_ripple.setChecked(bool(self.cfg.get("ripple_enabled", True)))
            self.spin_ripple_size.setValue(int(self.cfg.get("ripple_size_px",120)))
            self.spin_ripple_dur.setValue(int(self.cfg.get("ripple_duration_ms",450)))
    ...
    def _reset_cfg(self):
        ...
        self.chk_ripple.setChecked(True)
        self.spin_ripple_size.setValue(120)
        self.spin_ripple_dur.setValue(450)
