...
"""
AutoHotCorner v11
- Fix: Win, Tab 같은 시스템 단축키도 '조합키 지정'에서 정상 인식 (저수준 keyboard 훅 사용)
- 개선: 조합키 실행 시 keyboard.send() 우선 사용 (Win+Tab 등 시스템 키 조합 호환성↑)
- 참고: keyboard 모듈이 없거나 권한이 부족하면 기존 Qt 이벤트 방식/pyautogui 로 자동 폴백
Baseline: v10 (Effects)
"""
...
from PyQt5.QtCore import Qt, QTimer, QPoint, QRect, QEasingCurve, QEvent
...
# Try low-level keyboard hook (Windows)
HAS_KB = False
KB = None
...
VERSION = "11.0"
ICON_NAME = "AutoHotKeyIcon.ico"  # place next to script or exe
...
def default_config() -> dict:
    return {
        ...
        # Hotkeys per corner (3-key combos)
        "hotkeys": [
            ["none", "none", "none"],
            ["none", "none", "none"],
            ["none", "none", "none"],
        ...
        "effect_enabled": True,
        "effect_type": "Ripple",        # Ripple | SquareRipple | Flash | Crosshair
        "effect_size_px": 120,
        "effect_duration_ms": 450,
        "effect_color": "#5A82FF",
        "effect_stroke_px": 4,
        "effect_stroke_opacity": 60,    # %
        "effect_fill_opacity": 15       # % for ripple/square fill
    }
...
def load_config() -> dict:
    ...
    if p.exists():
        try:
            ...
            # hotkeys shape
            if len(base.get("hotkeys", [])) != 4:
                base["hotkeys"] = default_config()["hotkeys"]
            return base
        except Exception:
            pass
...
KB_SEND_MAP = {
    # mapping to keyboard library key names
    "win":"windows",
}

PYAUTO_MAP = {
    # pyautogui historically used 'winleft'; keep both
    "win":"winleft",
}
...
def _format_for_keyboard_lib(keys):
    parts = []
    for k in keys:
        k2 = KB_SEND_MAP.get(k, k)
        parts.append(k2)
    return "+".join(parts)

def pyauto_press_combo(keys):
    """Send key combo. Prefer keyboard.send if available; fallback to pyautogui."""
    # filter 'none'
    ...
    try:
        if HAS_KB and KB is not None:
            seq = _format_for_keyboard_lib(keys)
            KB.send(seq, do_press=True, do_release=True)
            return
    except Exception as e:
        # fallback to pyautogui
        print("keyboard.send 실패 → pyautogui로 폴백:", e)
    # pyautogui fallback
    try:
        for k in keys[:-1]:
            k2 = PYAUTO_MAP.get(k, k)
            pyautogui.keyDown(k2)
        pyautogui.press(PYAUTO_MAP.get(keys[-1], keys[-1]))
    finally:
        for k in reversed(keys[:-1]):
            try:
                pyautogui.keyUp(PYAUTO_MAP.get(k, k))
            except Exception:
                pass
...
class KeyCaptureDialog(QDialog):
    """
    조합키 입력 캡처.
    - Windows에서 keyboard 훅(HAS_KB=True)이면 Win/Tab 같은 시스템 키도 인식 (suppress=True)
      → Enter 로 확정, Esc 로 취소는 훅에서 직접 처리
    - 그 외 환경/폴백: 기존 Qt keyPressEvent 로 동작
    """
    def __init__(self, parent=None):
        ...
        self.lbl = QLabel("조합키를 누르고 Enter로 확정, ESC로 취소하세요.\n예) Ctrl+Alt+K, Win+Tab (최대 3키)")
        ...
        self._current = ["none"]
        self._last_non_empty = ["none"]
        self._max_combo = ["none"]  # 이번 누름 세션 동안 관측된 '가장 큰' 조합 (길이/주키 포함)
        self._pressing = False
        self._down_order = []  # 이번 세션에서 "누른 순서"를 유지
        self._result = ["none"]
        ...
        # keyboard hook state
        self._use_kb_hook = bool(HAS_KB and KB is not None and platform.system()=="Windows")
        self._pressed = set()
        self._hook = None
    ...
    def showEvent(self, e):
        ...
        if self._use_kb_hook and self._hook is None:
            # suppress=True to prevent system handling (e.g., Win+Tab)
            def _handler(ev):
                try:
                    name = (ev.name or "").lower()
                    if name == "left windows" or name == "right windows":
                        name = "win"
                    name = SPECIAL_MAP.get(name, name)
                    # Enter/ESC accept/reject handled here to allow suppress
                    if ev.event_type == "down":
                        if name in ("enter", "return"):
                            # finalize with current keys
                            # 확정 시: "누른 순서"를 우선 사용 (최대 3키).
                            seq = [k for k in self._down_order if k and k != "none"][:3]
                            if not seq:
                                # 폴백: 세션 중 최대 조합 → 현재 → 마지막 비-빈
                                seq = self._max_combo[:]
                                if seq == ["none"]:
                                    seq = self._current[:]
                                if seq == ["none"]:
                                    seq = self._last_non_empty[:]
                            self._result = seq
                            self.accept()
                            return
                    ...
                    # Build ordered list from currently pressed keys: modifiers first, then last non-modifier
                    ...
                    # Start or update a press session
                    if ev.event_type == "down":
                        if not self._pressing:
                            self._pressing = True
                            self._max_combo = ["none"]
                            self._down_order = []  # 새 세션 시작 시 리셋
                        if seq_now != ["none"]:
                            self._last_non_empty = seq_now
                            # Down 순서 기록 (중복 방지)
                            # ev.name → name 변환은 상단에서 이미 수행됨
                            k = name
                            if k not in self._down_order and k not in ("enter","return","esc","escape"):
                                self._down_order.append(k)
                            # Update max combo using a simple score: length + bonus for having a main key
                            def score(s):
                                base = len([x for x in s if x != "none"])
                                has_main = any(x not in ("ctrl","alt","shift","win") for x in s)
                                return base + (2 if has_main else 0)
                            if score(seq_now) > score(self._max_combo):
                                self._max_combo = seq_now
                    elif ev.event_type == "up":
                        # When a key is released, don't shrink the preview. Only finalize when EVERYTHING is up.
                        pass

                    # Preview policy:
                    # - While ANY key is pressed: always show the max combo seen in this session (no shrinking as keys are released)
                    # - When ALL keys are up: finalize by showing the max combo and ending the session
                    self._current = seq_now
                    if self._pressed:
                        # still pressing something → show max combo (peak)
                    ...
                    pass
            try:
                self._hook = KB.hook(_handler, suppress=True)
            except Exception:
                self._use_kb_hook = False  # fallback to Qt
    ...
    def _accept(self):
        seq = self._current
        if seq == ["none"]:
            # 현재가 비어있으면 세션 중 최대 조합 → 없으면 마지막 비-빈
            seq = self._max_combo if self._max_combo != ["none"] else self._last_non_empty
        self._result = seq
        self.accept()

    # Qt fallback path
    def keyPressEvent(self, e):
        if self._use_kb_hook:
            # handled by hook
            return
        key = e.key()
        mods = e.modifiers()
...
class EffectOverlay(QWidget):
    """
    핫코너 시각효과 오버레이.
    - configure(): 효과 파라미터 설정
    - show_effect(x, y, hold=False): 효과 시작. hold=True면 증가 애니메이션 이후 최대 크기/불투명도로 유지
    - hide_effect(): 즉시 숨김
    """
    def __init__(self, parent=None):
        ...
        self._duration_ms = 450
        self._elapsed = 0
        self._last_ms = None

        self._effect_type = "Ripple"
        self._color = QColor("#5A82FF")
        self._stroke_px = 4
        self._stroke_alpha = 0.6   # 0~1
        self._fill_alpha = 0.15    # 0~1

        self._hold = False  # hold 모드 여부
    ...
    def configure(self, *, effect_type:str, max_radius:int, duration_ms:int,
                  color, stroke_px:int, stroke_opacity_percent:int, fill_opacity_percent:int):
        """효과 파라미터 세팅"""
        self._effect_type = effect_type
        self._max_radius = max(20, int(max_radius))
        self._duration_ms = max(100, int(duration_ms))
        # color 인자가 문자열(hex)로 올 수도 있어 안전 변환
        if isinstance(color, QColor):
            self._color = color
        else:
            self._color = QColor(str(color))
        self._stroke_px = max(1, int(stroke_px))
    ...
    def show_effect(self, x:int, y:int, hold:bool=False):
        self._center = QPoint(int(x), int(y))
        self._radius = 0
        self._opacity = 1.0
        self._elapsed = 0
    ...
    def _tick(self):
        ...
        if not self._hold:
            # 일반 모드: 확대되며 사라짐
            self._radius = int(self._max_radius * eased)
            self._opacity = max(0.0, 1.0 - t)
        else:
            # hold 모드: t<1 동안은 커지기만 하고, 이후 최대 크기/완전 불투명 유지
            if t < 1.0:
                self._radius = int(self._max_radius * eased)
            else:
                self._radius = int(self._max_radius)
            self._opacity = 1.0

        self.update()

        if t >= 1.0 and not self._hold:
            self._timer.stop()
            self.hide()
    ...
    def paintEvent(self, ev):
        ...
        stroke_col = QColor(self._color.red(), self._color.green(), self._color.blue(),
                            int(255 * self._opacity * self._stroke_alpha))
        fill_col   = QColor(self._color.red(), self._color.green(), self._color.blue(),
                            int(255 * self._opacity * self._fill_alpha))

        if self._effect_type == "Ripple":
            pen = QPen(stroke_col, self._stroke_px)
            p.setPen(pen); p.setBrush(Qt.NoBrush)
            p.drawEllipse(self._center, self._radius, self._radius)

            p.setPen(Qt.NoPen); p.setBrush(QBrush(fill_col))
            inner = max(0, self._radius - max(8, self._stroke_px * 2))
            p.drawEllipse(self._center, inner, inner)

        elif self._effect_type == "SquareRipple":
            pen = QPen(stroke_col, self._stroke_px)
            p.setPen(pen); p.setBrush(Qt.NoBrush)
            side = self._radius * 2
            rect = QRect(self._center.x()-self._radius, self._center.y()-self._radius, side, side)
            p.drawRect(rect)

            p.setPen(Qt.NoPen); p.setBrush(QBrush(fill_col))
            inset = max(0, self._stroke_px * 2)
            rect2 = QRect(rect.adjusted(inset, inset, -inset, -inset))
            p.drawRect(rect2)

        elif self._effect_type == "Flash":
            pen = QPen(Qt.NoPen)
            p.setPen(pen); p.setBrush(QBrush(fill_col))
            size = max(20, int(self._radius * 1.2))
            points = [
                QPoint(self._center.x(), self._center.y()-size),
                QPoint(self._center.x()+size, self._center.y()),
                QPoint(self._center.x(), self._center.y()+size),
                QPoint(self._center.x()-size, self._center.y()),
            ]
            p.drawPolygon(*points)
            pen2 = QPen(stroke_col, self._stroke_px)
            ...
            p.setPen(Qt.NoPen); p.setBrush(QBrush(fill_col))
            p.drawEllipse(self._center, max(2, self._stroke_px), max(2, self._stroke_px))
        ...
        def configure(self, *, effect_type:str, max_radius:int, duration_ms:int,
            ...
            self._max_radius = max(20, int(max_radius))
            self._duration_ms = max(100, int(duration_ms))
            self._color = color if hasattr(color, 'red') else QColor(str(color))
            self._stroke_px = max(1, int(stroke_px))
            self._stroke_alpha = max(0, min(100, int(stroke_opacity_percent))) / 100.0
            self._fill_alpha = max(0, min(100, int(fill_opacity_percent))) / 100.0
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        try:
            w, h = self.cfg.get("window_size", [620, 630])
        ...
        hint = QLabel("팁: Ctrl/Alt/Shift/Win + Key 조합을 지원합니다. 최대 3키. (Win+Tab 등 가능)")
        ...
        ws = self.cfg.get("window_size", [620, 630])
        ...
        # effect options
        gb_effect = QGroupBox("핫코너 시각 효과")
        ge = QGridLayout(gb_effect)
        self.chk_effect = QCheckBox("효과 사용")
        self.chk_effect.setChecked(bool(self.cfg.get("effect_enabled", True)))

        self.combo_effect = QComboBox()
        for name in ["Ripple","SquareRipple","Flash","Crosshair"]:
            self.combo_effect.addItem(name)
        self.combo_effect.setCurrentText(self.cfg.get("effect_type","Ripple"))
        ...
        self.btn_pick_color = QPushButton("색상 선택")
        self.lbl_color_preview = QLabel("■")
        self.lbl_color_preview.setFixedWidth(22)
        self.lbl_color_preview.setAlignment(Qt.AlignCenter)
        self._effect_color = QColor(self.cfg.get("effect_color","#5A82FF"))
        ...
        self._inside_idx = None
        self._inside_visual = None
    ...
    def _update_color_preview(self):
        c = self._effect_color
        self.lbl_color_preview.setStyleSheet(f"font-size:18px;color:{c.name()};")

    def _pick_color(self):
        c = QColorDialog.getColor(self._effect_color, self, "효과 색상 선택")
        if c.isValid():
            self._effect_color = c
            self._update_color_preview()
    ...
    def _do_keepalive(self):
        ...
        try:
            # send twice
            pyauto_press_combo([key]); pyauto_press_combo([key])
    ...
    def _poll_mouse(self):
        ...
        # 상태 전이: None->hit(입장), inside->same(유지), inside->None/other(이탈→실행)
        if self._inside_idx is None:
            if hit_idx is not None:
                # ENTER → 효과(hold)
                self._inside_idx = hit_idx
                self._inside_visual = hit_pos
                if hit_pos is not None:
                    self._maybe_effect(hit_pos[0], hit_pos[1], hold=True)
        else:
            if hit_idx == self._inside_idx:
                # 여전히 inside → 효과는 hold 상태 유지
                pass
            else:
                # EXIT → 동작 실행 + 오버레이 숨김
                fire_idx = self._inside_idx
                vis = self._inside_visual
                self._inside_idx = None
                self._inside_visual = None
                try:
                    self.overlay.hide_effect()
                except Exception:
                    pass

                if vis is not None:
                    self._trigger_corner(fire_idx, visual_x=vis[0], visual_y=vis[1])
                else:
                    self._trigger_corner(fire_idx)
    ...
    def _maybe_effect(self, x:int, y:int, hold:bool=False):
        if not self.chk_effect.isChecked():
            return
        self.overlay.configure(
            ...
            color=self._effect_color,  # QColor 혹은 "#RRGGBB"
    ...
    def _trigger_corner(self, idx: int, visual_x=None, visual_y=None):
        # show effect first (if enabled)
        if visual_x is not None and visual_y is not None:
            self._maybe_effect(visual_x, visual_y)
    ...
    def _save_clicked(self):
        ...
        # effect
        self.cfg["effect_enabled"] = self.chk_effect.isChecked()
        self.cfg["effect_type"] = self.combo_effect.currentText()
        self.cfg["effect_size_px"] = int(self.spin_effect_size.value())
        self.cfg["effect_duration_ms"] = int(self.spin_effect_dur.value())
        self.cfg["effect_color"] = self._effect_color.name()
        self.cfg["effect_stroke_px"] = int(self.spin_effect_stroke.value())
        self.cfg["effect_stroke_opacity"] = int(self.spin_effect_stroke_op.value())
        self.cfg["effect_fill_opacity"] = int(self.spin_effect_fill_op.value())
    ...
    def _import_cfg(self):
        ...
        try:
            ...
            self.chk_effect.setChecked(bool(self.cfg.get("effect_enabled", True)))
            self.combo_effect.setCurrentText(self.cfg.get("effect_type","Ripple"))
            self.spin_effect_size.setValue(int(self.cfg.get("effect_size_px",120)))
            self.spin_effect_dur.setValue(int(self.cfg.get("effect_duration_ms",450)))
            self._effect_color = QColor(self.cfg.get("effect_color","#5A82FF"))
            self._update_color_preview()
            self.spin_effect_stroke.setValue(int(self.cfg.get("effect_stroke_px",4)))
            self.spin_effect_stroke_op.setValue(int(self.cfg.get("effect_stroke_opacity",60)))
            self.spin_effect_fill_op.setValue(int(self.cfg.get("effect_fill_opacity",15)))
    ...
    def _reset_cfg(self):
        ...
        self.chk_autominimize_tray = False
        ...
        self.combo_effect.setCurrentText("Ripple")
        self.spin_effect_size.setValue(120)
        self.spin_effect_dur.setValue(450)
        self._effect_color = QColor("#5A82FF")
        self._update_color_preview()
        self.spin_effect_stroke.setValue(4)
        self.spin_effect_stroke_op.setValue(60)
        self.spin_effect_fill_op.setValue(15)
