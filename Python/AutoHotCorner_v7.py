# -*- coding: utf-8 -*-
"""
AutoHotCorner v7 (PyQt5 single-file)
- Hotcorner detection: ALWAYS ON while app runs
- Keepalive: Start/Stop controls only periodic key (monitor sleep prevention)
- Immediate key send on Keepalive start
- UI/UX: Dark theme, tabs, tray icon, live log
- Key capture dialog supports combos (Ctrl/Alt/Shift/Win + Key); CapsLock/NumLock supported
- Calibration dialog for precise corner points
- Settings: export/import/reset
- Autorun (Windows), Autostart keepalive, Auto-minimize to tray
- NEW in v7:
  * Settings → Window Size (W/H) with "Use on next launch" (applied immediately on save too)
  * Settings → Custom "About" label toggle + editable text
"""
...
from PyQt5.QtWidgets import (
    ...
    QSystemTrayIcon, QMenu, QLineEdit, QFileDialog, QTabWidget, QStyle, QDialog, QDialogButtonBox, QTextEdit
)
...
def default_config() -> dict:
    return {
        ...
        # UI options (new)
        "window_size": [620, 400],  # default size (your requested change)
        "use_custom_about": True,
        "custom_about_text": "AutoHotCorner v7.0 — For Windows Hotcorner + Keepalive Monitor"
    }
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        # Info
        card2 = QGroupBox("실시간 정보 (핫코너는 항상 감지 중)")
        ...
        # custom about (new)
        self.chk_custom_about = QCheckBox("사용자 지정 About 텍스트 사용")
        self.chk_custom_about.setChecked(bool(self.cfg.get("use_custom_about", False)))
        self.edit_custom_about = QTextEdit()
        self.edit_custom_about.setFixedHeight(60)
        self.edit_custom_about.setPlaceholderText("예) AutoHotCorner v7.0 — For Windows Hotcorner + Keepalive Monitor")
        self.edit_custom_about.setText(self.cfg.get("custom_about_text", ""))
        ...
        gl3.addWidget(self.chk_autorun, 2, 0, 1, 2)
        gl3.addWidget(self.chk_autostart_keepalive, 3, 0, 1, 2)
        gl3.addWidget(self.chk_autominimize, 4, 0, 1, 2)

        gl3.addWidget(QLabel("창 크기 W x H:"), 5, 0)
        gl3.addWidget(self.spin_win_w, 5, 1)
        gl3.addWidget(self.spin_win_h, 5, 2)
        gl3.addWidget(self.btn_apply_current_size, 5, 3)

        gl3.addWidget(self.chk_custom_about, 6, 0, 1, 4)
        gl3.addWidget(self.edit_custom_about, 7, 0, 1, 4)

        v4.addWidget(gb3)
        about = QGroupBox("About")
        ab_l = QVBoxLayout(about)
        about_text = self._about_text()
        self.lbl_about = QLabel(about_text)
    ...
    def _about_text(self) -> str:
        use_custom = bool(self.cfg.get("use_custom_about", False))
        if use_custom:
            txt = self.cfg.get("custom_about_text") or ""
            if txt.strip():
                return txt
        return f"{APP_NAME} v{VERSION} — 윈도우에서 핫코너는 항상 동작합니다. Keepalive만 On/Off."
        ...
        for corner_idx, entries in checks.items():
            for mon, ax, ay in entries:
                if match(ax, ay):
                    ...
                    self._cooldown = True
                    QTimer.singleShot(500, lambda: setattr(self, "_cooldown", False))
                    return
    ...
    def _save_clicked(self):
        ...
        self.cfg["use_custom_about"] = self.chk_custom_about.isChecked()
        self.cfg["custom_about_text"] = self.edit_custom_about.toPlainText()
        ...
        try:
            ...
            pass
        self.lbl_about.setText(self._about_text())
    ...
    def _reset_cfg(self):
        ...
        self.chk_custom_about.setChecked(self.cfg.get("use_custom_about", True))
        self.edit_custom_about.setText(self.cfg.get("custom_about_text", ""))
        self.lbl_about.setText(self._about_text())
