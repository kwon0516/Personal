...
class EffectOverlay(QWidget):
    """A frameless, translucent overlay that renders different visual effects at (x,y)."""
    def __init__(self, parent=None):
        ...
        self._stroke_px = 4
        self._stroke_alpha = 0.6   # 60%
        self._fill_alpha = 0.15    # 15%

        # Fullscreen overlay across virtual desktop
    ...
    def configure(self, *, effect_type:str, max_radius:int, duration_ms:int,
                  color:QColor, stroke_px:int, stroke_opacity_percent:int, fill_opacity_percent:int):
    ...
    # --- Animate ---
    def show_effect(self, x:int, y:int):
...
class MainWindow(QMainWindow):
    ...
    def _poll_mouse(self):
        ...
        for corner_idx, entries in checks.items():
            for mon, ax, ay in entries:
                if match(ax, ay):
                    self._trigger_corner(corner_idx, visual_x=ax, visual_y=ay)
                    self._cooldown = True
    ...
    def _maybe_effect(self, x:int, y:int):
        ...
        self.overlay.show_effect(int(x), int(y))
