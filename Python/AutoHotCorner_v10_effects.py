# -*- coding: utf-8 -*-
"""
AutoHotCorner v10 (Effects)
Baseline: v9 final (hotcorner always-on + keepalive) + v9 ripple
New in v10:
  - 시각 효과 종류 옵션화: Ripple(원형), SquareRipple(사각), Flash(코너 번쩍), Crosshair(십자선)
  - 효과 크기/지속시간/색상/불투명도/선두께 등 UI 설정 추가
  - 효과 비활성화 옵션
  - 기존 기능 모두 유지 (앱 실행/단축키/상호배타/쿨다운/자동시작/트레이/설정 저장/가져오기/내보내기)
"""
...
class EffectOverlay(QWidget):
    ...
    def paintEvent(self, ev):
        ...
        if self._effect_type == "Ripple":
            ...
            # Flash the whole corner region (quarter triangle from (0,0) to center) — we base the corner by mouse quadrant
            # Simpler: draw a quick diamond pulse at the point
            pen = QPen(Qt.NoPen)
